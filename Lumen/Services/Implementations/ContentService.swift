...
final class ContentService: ContentServiceProtocol {
    ...
    func loadBundledContentIfNeeded(modelContext: ModelContext) throws {
        ...
        for cat in allCategories {
            if seenCatIds.contains(cat.id) {
                modelContext.delete(cat)
            } else {
                seenCatIds.insert(cat.id)
            }
        }
        ...
        // Remove additional duplicates generated by old loads
        try deduplicateAffirmations(modelContext: modelContext)
        try deduplicateThemes(modelContext: modelContext)
        
        try modelContext.save()
        logger.info("Bundled content loaded.")
    }
    
    private func deduplicateAffirmations(modelContext: ModelContext) throws {
        let allItems = try modelContext.fetch(FetchDescriptor<Affirmation>())
    ...
    private func deduplicateThemes(modelContext: ModelContext) throws {
        let allItems = try modelContext.fetch(FetchDescriptor<AppTheme>())
        var seenIds = Set<String>()
        for item in allItems {
            if seenIds.contains(item.id) {
                modelContext.delete(item)
